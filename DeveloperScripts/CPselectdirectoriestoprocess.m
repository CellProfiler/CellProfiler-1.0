function handles = CPselectdirectoriestoprocess(handles)
...
if ishandle(SelectDirectoryFig)
    ...
    if iscell(Choice)       % Multiple entries
        Selection = logical(cat(1,Choice{:}));
    elseif isscalar(Choice) % Only one entry
        Selection = logical(Choice);
    else
...
SelectDirectoryFig = CPfigure('Units','Inches','Resize','Off','Menubar','None','Toolbar','None','NumberTitle','Off','Name','Select Display Window','Color',CPBackgroundColor(),'UserData',0);
