...
pretty_field_name(find(field_name == '_')) = ' ';
MeasurementToExtract = sprintf('%s of %s', field_name, object_name);

%%% Put the measurements for this feature in a cell array, one
%%% cell for each cycle.
Measurements = handles.Measurements.(object_name).(field_name);
...
% The LoadText module allows the user to load a text file that assigns
% a label (e.g., a chemical name) to each image.  There could
% potentially be several such sets of labels loaded (when there is
% more than one LoadText module in the pipeline).  The following
% promts the user to choose one of them, then loads the labels.  They
% will be used later when the histograms are displayed.

prefix = 'LoadedText_';
sample_info_sets = ImportedFieldnames(strmatch(prefix, ImportedFieldnames));
for i = 1:length(sample_info_sets)
    % Trim off the first part of the name.
    sample_info_sets{i} = sample_info_sets{i}(length(prefix)+1:end);
end

if ~isempty(sample_info_sets)
    [Selection, ok] = CPlistdlg('ListString',sample_info_sets, 'ListSize', [300 400],...
        'Name','Select sample info',...
        'PromptString','Choose the sample info with which to label each histogram.','CancelString','Cancel',...
        'SelectionMode','single');
    if ok ~= 0
        HeadingName = char(sample_info_sets(Selection));
        try SampleNames = handles.Measurements.Image.([prefix HeadingName]);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if strcmpi(UserInput.Combine, 'Yes') == 1
    ...
    if strcmpi(UserInput.Logical,'None') == 1
        ...
        %%% altered.
    elseif strcmpi(UserInput.Logical,'>') == 1
    ...
    if strcmpi(CalcStatistics,'Yes')
        ...
        uicontrol(StatsFigHandle,'style','text','units','normalized',...
            ...
            'Backgroundcolor',CPBackgroundColor(),'fontname','Helvetica',...
...
if strcmpi(UserInput.BinVar,'Percentages')
...
if strcmpi(UserInput.ExportHist,'Yes') == 1
...
if ~strcmpi(UserInput.Style,'Heatmap') && strcmpi(UserInput.Display,'Yes')
    ...
    for ImageNumber = UserInput.FirstSample:UserInput.LastSample
        ...
        if strcmpi(UserInput.Style,'Bar')
            if ~isempty(FinalHistogramData) && strcmpi(UserInput.Xaxis,'Number of objects in bin')
                k=barh('v6',PlotBinLocations,FinalHistogramData(:,ImageNumber), 1);
            elseif ~isempty(FinalHistogramData) && strcmpi(UserInput.Xaxis,'Measurements')
                k=bar('v6',PlotBinLocations,FinalHistogramData(:,ImageNumber), 1);
            end
            set(k,'FaceColor',GraphColor);
        elseif strcmpi(UserInput.Style,'Area')
        ...
        if strcmpi(UserInput.Xaxis,'Number of objects in bin') && ~strcmpi(UserInput.Style,'Area')
            if Increment == 1
    ...
    if strcmpi(UserInput.RelAbs, 'Relative number of objects') == 1
    ...
    elseif strcmpi(UserInput.RelAbs, 'Absolute number of objects') == 1
    ...
    %%% window.
    if strcmpi(computer,'MAC') && str2double(VersionCheck(1:3)) < 7.1
        ...
        if strcmpi(UserInput.Style,'Line')
    ...
    if ~isdeployed      %button should only be shown in developer's version
        ...
        if strcmpi(UserInput.Style,'Bar') || strcmpi(UserInput.Style,'Area')
...
elseif strcmpi(UserInput.Style,'Heatmap') == 1 && strcmpi(UserInput.Display,'Yes') == 1
    ...
    if strcmpi(UserInput.EachRow,'Image') == 1
        ...
        if strcmpi(UserInput.BinVar,'Percentages') == 1
    ...
    if strcmpi(UserInput.EachRow,'Bin')
...
okbutton = uicontrol(FigHandle,'style','pushbutton','String','OK','Fontweight','bold','FontSize',FontSize,'units','pixels',...
    'position',[posx 10 75 30],'BackgroundColor',CPBackgroundColor(),'Callback','[cobj,cfig] = gcbo;set(cobj,''UserData'',1);uiresume(cfig);clear cobj cfig;','BackgroundColor',CPBackgroundColor());
cancelbutton = uicontrol(FigHandle,'style','pushbutton','String','Cancel','Fontweight','bold','FontSize',FontSize,'units','pixels',...
    'position',[posx+100 10 75 30],'Callback','close(gcf)','BackgroundColor',CPBackgroundColor()); %#ok Ignore MLint
...
if ~isempty(UserInput)
    try %#ok Ignore MLint
        ...
        if strcmpi(UserInput.LeftBin,'Min value found')
            set(LeftBin,'value',1);
        else
            set(LeftBin,'value',2);
        end
        ...
        if strcmpi(LeftValNumber,'NaN')
            LeftValNumber='';
        end
        set(LeftVal,'string',LeftValNumber);

        if strcmpi(UserInput.RightBin,'Max value found')
        ...
        if strcmpi(RightValNumber,'NaN')
            RightValNumber='';
        end
        set(RightVal,'string',RightValNumber);

        if strcmpi(UserInput.Logical,'None')
            set(Logical,'value',1);
        elseif strcmpi(UserInput.Logical,'>')
            set(Logical,'value',2);
        elseif strcmpi(UserInput.Logical,'>=')
            set(Logical,'value',3);
        elseif strcmpi(UserInput.Logical,'=')
        ...
        if strcmpi(ThresholdValNumber,'NaN')
            ThresholdValNumber='';
        end
        set(ThresholdVal,'string',ThresholdValNumber);

        if strcmpi(UserInput.Combine,'No')
            set(Combine,'value',1);
        else
            set(Combine,'value',2);
        end

        if strcmpi(UserInput.Xaxis,'Measurements')
            set(Xaxis,'value',1);
        else
            set(Xaxis,'value',2);
        end

        if strcmpi(UserInput.RelAbs,'Relative number of objects')
            set(RelAbs,'value',1);
        else
            set(RelAbs,'value',2);
        end

        if strcmpi(UserInput.Log,'No')
            set(Log,'value',1);
        else
            set(Log,'value',2);
        end
        ...
        if strcmpi(UserInput.Style,'Bar')
            set(Style,'value',1);
        elseif strcmpi(UserInput.Style,'Line')
            set(Style,'value',2);
        elseif strcmpi(UserInput.Style,'Area')
            set(Style,'value',3);
        elseif strcmpi(UserInput.Style,'Heatmap')
            set(Style,'value',4);
        end

        if strcmpi(UserInput.Color,'Blue')
            set(Color,'value',1);
        elseif strcmpi(UserInput.Color,'Red')
            set(Color,'value',2);
        elseif strcmpi(UserInput.Color,'Green')
            set(Color,'value',3);
        elseif strcmpi(UserInput.Color,'Yellow')
            set(Color,'value',4);
        elseif strcmpi(UserInput.Color,'Magenta')
            set(Color,'value',5);
        elseif strcmpi(UserInput.Color,'Cyan')
            set(Color,'value',6);
        elseif strcmpi(UserInput.Color,'Black')
            set(Color,'value',7);
        elseif strcmpi(UserInput.Color,'White')
            set(Color,'value',8);
        elseif strcmpi(UserInput.Color,'CellProfiler background')
            set(Color,'value',9);
        end

        if strcmpi(UserInput.Display,'Yes')
            set(Display,'value',1);
        else
            set(Display,'value',2);
        end

        if strcmpi(UserInput.ExportHist,'No')
        ...
        if strcmpi(UserInput.EachRow,'Image')
            set(EachRow,'value',1);
        else
            set(EachRow,'value',2);
        end
