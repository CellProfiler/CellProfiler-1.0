...
if any(findobj == ThisModuleFigureNumber)
    ...
    if AreThereThreeInputImages,
        if wantImagesCropped,
            ...
            OriginalRGB = cat(3,sum(padarray(ModifiedImages{3},[Mi-M3 Ni-N3],'pre'),3)/sum(max(max(ModifiedImages{3}))),...
            ...
            AlignedRGB = cat(3, sum(padarray(AlignedImages{3},[Mi-M3 Ni-N3],'pre'),3)/sum(max(max(AlignedImages{3}))),...
